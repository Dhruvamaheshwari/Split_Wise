      ...
      <main className="max-w-5xl mx-auto p-6 w-full flex-1">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {groups.length === 0 ? (
              <div className="col-span-full flex flex-col items-center justify-center text-center py-24 bg-white dark:bg-slate-800 rounded-[2.5rem] border border-gray-100 dark:border-slate-700 shadow-sm daisy-card">
                ...
                <h3 className="font-heading text-2xl font-black text-gray-900 dark:text-white mb-3">No groups yet</h3>
                <p className="text-gray-500 dark:text-gray-400 font-medium mb-8 max-w-sm">Start tracking expenses, splitting bills, and managing your finances with friends.</p>
                ...
                >
                  <div onClick={() => navigate(`/group/${group.id}`)} className="flex-1 cursor-pointer">
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-12 h-12 bg-gray-50 dark:bg-slate-700/50 rounded-2xl flex items-center justify-center group-hover:bg-primary-50 dark:group-hover:bg-slate-700 transition-colors">
                        <span className="text-xl font-black text-primary-600">{group.name.charAt(0).toUpperCase()}</span>
                      </div>
                      <span className="font-mono-num text-xs font-bold bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-300 px-3 py-1 rounded-full">
                        {group.members?.length || 0} Members
                      </span>
                    </div>
                    <h3 className="font-heading text-xl font-extrabold text-gray-900 dark:text-white mb-4 truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{group.name}</h3>
                    
                    <div className="flex -space-x-3 mb-6">
                      {group.members?.slice(0, 4).map((m, i) => (
                        <div key={i} className="w-10 h-10 rounded-full bg-gradient-to-br from-gray-100 dark:from-slate-600 to-gray-200 dark:to-slate-700 border-2 border-white dark:border-slate-800 flex items-center justify-center text-sm text-gray-700 dark:text-gray-200 font-bold shadow-sm">
                          {(m.user?.username || m.user?.email || '?').charAt(0).toUpperCase()}
                        </div>
                      ))}
                      {group.members?.length > 4 && (
                        <div className="w-10 h-10 rounded-full bg-gray-50 dark:bg-slate-700 border-2 border-white dark:border-slate-800 flex items-center justify-center text-xs text-gray-500 dark:text-gray-300 font-bold shadow-sm">
                          +{group.members.length - 4}
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="pt-4 border-t border-gray-50 dark:border-slate-700/50 mt-auto flex justify-between items-center">
